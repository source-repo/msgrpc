...
export const runCall = (
    ...
    withNetwork(options, io, async (connected) => {
        ...
        try {
            ...
            const remote = options.idempotencyKey ? proxy.$with({ idempotencyKey: options.idempotencyKey }) : proxy
...
export const runWatch = (
    ...
    withNetwork(options, io, async (connected) => {
        ...
        try {
            ...
            await proxy.on(split.member, handler)
        } catch (e) {
        ...
        await proxy.off(split.member, handler).catch(() => undefined)
        return 0
    })
