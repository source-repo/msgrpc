...
const select = async (peer) => {
  ...
  for (const ns of described.namespaces) {
    ...
    const events = ns.events.map((e) =>
      '<div class="row"><code>' + esc(e.name) + '(' + (e.params ? e.params.map(typeText).join(', ') : '…') + ')</code>' +
      '<button data-watch="' + esc(ns.name) + '|' + esc(e.name) + '">watch</button>' +
      '<span class="muted">' + e.subscribers + ' subscriber' + (e.subscribers === 1 ? '' : 's') + '</span></div>').join('')
...
$('detail').addEventListener('click', async (e) => {
  ...
  const watchKey = e.target.dataset?.watch
  if (callKey) {
  ...
  if (watchKey) {
    const [namespace, event] = watchKey.split('|')
    e.target.disabled = true
    e.target.textContent = 'watching'
    await fetch('/api/watch', { method: 'POST', body: JSON.stringify({ peer: selected, namespace, event }) })
  }
})
