...
import { decodeHostStatePayload, deviceCommandTopicFilter, deviceTopic, encodeHostStatePayload, hostStateTopic, nodeTopic, parseSparkplugTopic } from './Types.js'

test('topics are built in the Sparkplug namespace', (t) => {
    t.is(nodeTopic('NBIRTH', { groupId: 'plant-a', edgeNodeId: 'edge-01' }), 'spBv1.0/plant-a/NBIRTH/edge-01')
    t.is(deviceTopic('DDATA', { groupId: 'plant-a', edgeNodeId: 'edge-01', deviceId: 'pump-7' }), 'spBv1.0/plant-a/DDATA/edge-01/pump-7')
    t.is(deviceCommandTopicFilter({ groupId: 'plant-a', edgeNodeId: 'edge-01' }), 'spBv1.0/plant-a/DCMD/edge-01/+')
...
test('topics parse back to their addresses', (t) => {
    t.deepEqual(parseSparkplugTopic('spBv1.0/plant-a/NBIRTH/edge-01'), {
        namespace: 'spBv1.0',
        type: 'NBIRTH',
        groupId: 'plant-a',
        edgeNodeId: 'edge-01'
    })
    t.deepEqual(parseSparkplugTopic('spBv1.0/plant-a/DDATA/edge-01/pump-7'), {
        namespace: 'spBv1.0',
        type: 'DDATA',
        groupId: 'plant-a',
        edgeNodeId: 'edge-01',
